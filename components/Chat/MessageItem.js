...
const MessageItem = ({
  ...
  const renderMessageContent = () => {
    if (msg.isRevoke) {
      return (
        <Text style={[textColor, { fontStyle: "italic" }]}>
  ...
  const renderReplyToMessage = () => {
    ...
    const isCurrentUserReply =
      String(msg.replyTo.sender?._id || msg.replyTo.sender) === String(userId);
    const senderName = isCurrentUserReply
      ? "You"
      : msg.sender.fullName.split(" ").pop();
    ...
    return (
      <TouchableOpacity
        style={[
          ...
          isCurrentUser
            ? styles.replyToContainerMe
            : styles.replyToContainerOther,
        ]}
    ...
    >
      ...
      >
        <TouchableOpacity
          activeOpacity={0.7}
          onPress={() =>
            msg.replyTo && !msg.isRevoke && onFocus(msg.replyTo._id, false)
          } // Chỉ cho phép nhấn vào reply nếu tin nhắn chưa bị thu hồi
          ...
          <View style={styles.modalOverlay}>
            <View
              style={[
                ...
                isCurrentUser ? styles.menuRight : styles.menuLeft,
              ]}
            >
              <TouchableOpacity
                style={styles.menuItem}
                onPress={() => handleMenuOption("reply")}
              >
              ...
              <TouchableOpacity
                style={styles.menuItem}
                onPress={() => handleMenuOption("forward")}
              >
              ...
              {isCurrentUser && !msg.isRevoke && (
                <TouchableOpacity
                  style={styles.menuItem}
                  onPress={() => handleMenuOption("recall")}
                >
